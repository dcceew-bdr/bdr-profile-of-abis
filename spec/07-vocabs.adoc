...
There are also elements of the models defined in this Profile that must be used with vocabularies, such as the <<annex-c-2-supporting-assets, Submission Manifest Resource Roles vocabulary>>'s `hasRole` predicate which must indicate a value from the <<annex_c_2_supporting_assets, Submission Manifest Resource Roles vocabulary>>.
